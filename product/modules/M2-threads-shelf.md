...
| **Closing Notes** | Finish/retire | — (owned by M3) | Creation + confirm flow is owned by M3 (see M3 §4, §8); M2 stores the confirmed note and displays it in the Finished & Retired gallery and in-story after restore. |
...
- **Emits** (names per the product-brief event dictionary): `thread.created` (new, proposal-adopted, and import Threads), `thread.state_changed(state)` (rest/rekindle/finish/retire and restore-to-warm — M3 Briefing scaling, M5 Doorway pool, billing auto-pause idle signal), `thread.merged` (Digest rebuild queue, M4 Arc reconciliation), `spark.moved` (single and bulk moves — M1 training signal). Digest freshness and Closing-Note availability are internal signals read by M3/M5, not bus events.
