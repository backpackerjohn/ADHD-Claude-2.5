...
- **100× / 10k+ Sparks (heavy importer or multi-year user):** all Spark lists virtualized; Thread views keep 50/page with time-bucket jump ("last spring"); Loose Sparks tray still renders 20 + fold regardless of backlog size. Classifier candidate list caps at 60 Threads — warm + recently-touched first, resting Threads represented by cluster one-liners — so filing latency and cost never grow with corpus size. Old filed Sparks are cold storage: hydrated on Thread open, not resident.
