...
- **Prompt contract (input):** cached stable prefix = system instructions + the user's Thread candidate list (name + one-line Digest summary each); injected **after the cache breakpoint**: the 10 similarity-selected correction exemplars (see below; canonical spec `ai-spec` §6) + the Spark text and `captured_at`. One call per Spark; imports batch ~20 Sparks per call.
- **Output contract (JSON, strict):** the merged filing schema of `ai-spec` §2.1 — `{"decision": "file" | "propose_new_thread" | "loose", "thread_id"?, "alt_thread_ids": [≤2], "reason": ≤12 words, "confidence": 0.0–1.0}`. Malformed or non-JSON output ⇒ treated as `loose` (fail safe, never fail wrong).
- **Confidence gate (R3 mitigation, canonical thresholds):** confidence **≥ 0.80** → auto-file with visible toast naming the Thread (filing is always announced, never silent). **0.55–0.79** → file with an inline "not right? drag it" affordance on the toast. **< 0.55** → Loose Sparks tray, **never a hidden inbox** ("out of sight, out of mind is a neurological reality" — [Medium/Brunell](https://raymond-brunell.medium.com/i-deleted-47-productivity-apps-in-30-days-heres-what-actually-worked-for-my-adhd-brain-52c292c6ba6b)). First 7 days per user: all thresholds **+0.05** (week-one conservative mode), because early misfiles are the trust-killer.
- **Corrections as training:** every one-drag correction writes an exemplar to the per-user store (capped at 200); the filing prompt injects the **10 most relevant exemplars** (embedding-similarity to the incoming Spark, recency-weighted), placed **after the cache breakpoint** so the cached prefix stays stable — canonical spec: `ai-spec` §6. No fine-tuning in v1.
...
- **Emits** (names per the product-brief event dictionary): `spark.captured` (with `night_captured` and import-source flags — M5 routing/counters, ember-hours cross-cutting layer), `spark.filed` (M2 Thread story; nightly Digest queue; a `loose` decision routes the tray, which M5 may resurface at most once per Doorway), `spark.corrected` / `spark.moved` (classifier exemplar store; M2 story), `thread.created` (adopted proposals, cold-start auto-threads, import clusters — M3 may offer Warm Starts on revived Threads; M2 refreshes Shelf). Import-batch completion is ImportBatch-internal state, not a bus event.
- **Consumes:** `thread.created` / `thread.state_changed(state)` / `thread.merged` from M2 (candidate-list maintenance; renames arrive via M2 metadata sync, not events); Digest one-liners from M2's digest job (classifier context); M5's morning-window signal (release of night Sparks and offline summary toast).
